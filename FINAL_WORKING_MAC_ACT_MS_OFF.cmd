...
set url=https://doymakey123.github.io/msmacchecker/
set keyword="5C-A6-E6-85-ED-DB"

:: Fetch the webpage content silently and store it in a temporary file
curl -s %url% > temp.txt

:: Search for the keyword in the webpage content
findstr /i %keyword% temp.txt

if %errorlevel%==0 (
	echo Data exists on the website.
) else (
	echo Data not exists on the website.
	exit
)
